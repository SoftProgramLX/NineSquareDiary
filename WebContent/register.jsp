...
<script language="javascript">
	...
	function Regopen(divID) {
		...
		divID.style.left = (document.body.clientWidth - 663)
		/2;		/ / 设置页面的左边距
		divID.style.top = (document.body.clientHeight - 441)
		/2;		/ / 设置页面的顶边距
	}
	//隐藏用户注册页面
	...
	function checkUser(str) {
		if (str == "") { //当用户名为空时
			...
			var loader = new net.AjaxRequest(
					"UserServlet?action=checkUser&username=" + str
							+ "&nocache=" + new Date().getTime(), deal,
					onerror, "GET");
		}
	}
	function deal() {
...
<script language="javascript">
	...
	function save() {
		...
		if (flag_user && flag_pwd && flag_repwd && flag_email && flag_question
				&& flag_answer) { //所有数据都符合要求时
			var param = "user=" + form1.user.value + "&pwd=" + form1.pwd.value
					+ "&email=" + form1.email.value + "&question="
					+ form1.question.value + "&answer=" + form1.answer.value
					+ "&city=" + form1.city.value; //组合参数
			var loader = new net.AjaxRequest("UserServlet?action=save&nocache="
					+ new Date().getTime(), deal_save, onerror, "POST", param);
		} else {
	...
	function getProvince() {
		var loader = new net.AjaxRequest(
				"UserServlet?action=getProvince&nocache="
	...
	function getCity(selProvince) {
		var loader = new net.AjaxRequest(
				"UserServlet?action=getCity&parProvince=" + selProvince
							...
							<td height="310" valign="top" bgcolor="#FFFFFF"><table
									...
									<tr>
										...
										<td height="40" align="center"><input name="btn_sumbit"
											...
											&nbsp; <input name="btn_reset" type="button" class="btn_grey"
											value="重置" onClick="form_reset(this.form)"> &nbsp; <input
